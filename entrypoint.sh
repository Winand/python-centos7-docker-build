...
if [ -f "$INSTALL_PATH/lib/pkgconfig/openssl.pc" ]; then
    LIB_SSL=lib
elif [ -f "$INSTALL_PATH/lib64/pkgconfig/openssl.pc" ]; then
    LIB_SSL=lib64
else
...
./configure --help | grep -q -e --with-openssl-rpath >/dev/null \
    && SSL_RPATH="--with-openssl-rpath=auto" \
    || SSL_RPATH="LDFLAGS=-Wl,-rpath=$INSTALL_PATH/$LIB_SSL"
./configure --enable-optimizations $NO_STATICLIB $NO_TESTMOD --prefix=$INSTALL_PATH \
            $SSL_RPATH PKG_CONFIG_PATH=$INSTALL_PATH/$LIB_SSL/pkgconfig
make
make altinstall
popd
